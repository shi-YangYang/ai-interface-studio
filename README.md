...
由协调 Agent 管理阶段门，分别委派产品与 UX、视觉设计、前端实施和视觉验收 Agent；用 Image Generation 生成设计系统与页面级 mockup，输出 `uiux-design.md`，并在前端实现前确认技术栈与视觉实现策略，最后可构建经过截图对照验收的交互式前端原型。
...
| 图片策略 | 单视口页面 1 张；长页面只生成 1 张竖向长页面基准图；另加 1 张绑定后续页面的设计系统图 |
| 文档策略 | 单一主文档作为精确文案、字段、状态和前端预览规则来源 |
| 协作策略 | 协调、产品与 UX、视觉设计、前端实施、视觉验收角色分离 |
| 技术策略 | 前端预览实施前确认框架、UI 库、图表/绘图/3D 库、素材和自定义组件 |
...
AI Interface Studio Skill 默认采用协调 Agent 驱动的常驻五角色协作模式，并在返修时按需启用前端修复 Agent。

| 角色 | 核心职责 | 明确禁止 |
| --- | --- | --- |
| 协调 Agent | 用户沟通、阶段门、任务分派、冲突决策、返修派单 | 不出最终 UI 图，不写或修改前端代码，不改预览素材，不自验收 |
| 产品与 UX Agent | 需求、流程、信息架构、页面与状态设计 | 不生成最终视觉图，不写前端代码 |
| 视觉设计 Agent | 设计系统、页面出图、视觉还原契约 | 不写前端代码 |
| 前端实施 Agent | 按批准图和契约实现前端预览 | 不修改设计来源，不自我验收 |
| 前端修复 Agent | 按 `VIS-###` 问题修复指定前端文件 | 不扩大范围，不改设计来源，不自我验收 |
| 视觉验收 Agent | 同视口截图对照、记录 `VIS-###` 问题、通过或拒收 | 不修改前端代码 |

各 Agent 通过磁盘上的批准产物交接，而不是依赖协调 Agent 对长对话的摘要。前端实施、前端修复和视觉验收 Agent 都必须查看原始 UI 图片。协调 Agent 只负责决策、派单、记录和推进流程，不允许直接修改前端预览代码。
...
| G4 实施策略确认 | 用户批准前端框架、UI 库、样式系统、图表/绘图/3D 库、素材计划和视觉技术映射 |
| G5 前端待验收 | 前端可运行，路由、目标视口截图和交互已准备 |
| G6 视觉验收通过 | 每页有参考图和浏览器截图证据，没有未关闭的 Blocker/Major `VIS-###` 问题 |
...
- **页面滚动**：页面内容超过一个视口，只生成 1 张竖向长页面基准图，不再生成横向 overview、顶部、中段、底部等独立细节 mockup。
- **内部滚动**：应用外壳保持固定，表格、画布或面板内部滚动，生成稳定工作区视图并在文档中说明滚动区域。

长页面不会只生成首屏。确认阶段会先列出每个页面的完整内容顺序、覆盖策略、预计图片数量、参考浏览器宽度和预计长图比例；竖向长图必须覆盖全部主要区块。

竖向长页面基准图是滚动页唯一的页面级 UI 图。它对应真实浏览器长页面，而不是横向设计板或缩略总览图；如果生成结果变成横向图、拼贴图或同尺寸压缩图，必须重生成。

滚动页的实现按比例契约还原：长图像素宽度映射到参考浏览器 CSS 宽度，长图像素高度按同一比例映射到浏览器完整页面截图高度。如果需要局部观察，只能对已批准竖向长图做确定性裁切或放大，不能重新生成局部 UI 图。如果同时构建前端预览，Codex 会用浏览器完整页面截图按比例对照长图。

默认软上限改为 **8 张业务页面设计帧 + 1 张设计系统图**。长页面基准图计为 1 帧；超过时 Codex 会先让用户确认优先级或分批生成。
...
- 组件规范
- 响应式说明
- 前端实施策略：框架、UI 库、样式系统、图标、图表/绘图/3D 库、动效、素材、自定义组件和参考图复刻映射
- 前端预览规格
...
视觉还原契约、前端实施策略和参考图复刻映射必须在编写前端代码前冻结。

### 6. 前端实施策略确认

在编写前端预览代码前，协调 Agent 会先分析已批准 UI 图和视觉还原契约，提出实施策略并等待用户确认。这个阶段用于把任务从“做一个相似原型”收紧为“按批准截图复刻”，避免把复杂视觉效果简化成普通卡片或模板页面。

实施策略会明确：

- 前端框架和运行时，例如 React、Vue、Vite 等
- UI 组件库，例如 Element Plus、Ant Design、MUI、Radix/shadcn 等
- 样式系统，例如 Tailwind CSS、CSS Modules、SCSS 或项目既有方案
- 图标库和自定义图标策略
- 图表、绘图、流程图、地图、蓝图叠加和数据可视化方案，例如 ECharts、D3、SVG、Canvas
- 3D、材质、粒子或空间效果方案，例如 Three.js 或 React Three Fiber
- 动效方案，例如 CSS transition、Framer Motion、GSAP
- 支撑素材计划，例如生成背景图、真实图片、纹理、遮罩或 SVG overlay
- 必须自定义实现的组件与高风险视觉区域
- 参考图复刻映射：每个页面区域、可见元素组、特殊效果、参考证据、实现方法、匹配规则和是否允许省略

每个主要页面区域都会形成视觉技术映射和 `RECON-###` 复刻清单：该区域参考哪张 UI 图、有哪些可见元素必须出现、用什么实现方法、依赖什么库或素材、是否存在还原风险。默认不允许省略 UI 图中的可见主要元素；任何省略或降级都必须由用户批准。用户批准后，前端实施 Agent 才能开始写代码。
...
前端实施 Agent 必须先查看全部原始 UI 图，并遵循已批准的前端实施策略和 `RECON-###` 复刻清单。它需要先实现并检查共享应用外壳，再分小批页面实现。预览使用 mock 数据、fixtures 和本地状态，并将 fixtures、mock services、UI types 与视觉组件分离。

如果 UI 图包含 3D 效果、工程蓝图叠加、复杂图表、数据可视化、地图、Canvas/SVG 绘制、特殊材质或动效，前端预览不能用普通占位卡片替代，除非该替代方案已在实施策略中被用户明确批准。
...
- 应用外壳、导航、内容边界与固定区域
- 页面结构、区域顺序、列宽比例和留白分布
- 长页面基准图与浏览器完整页面截图按比例映射后的滚动连续性
- 组件类型、数量、位置、尺寸与内容密度
- 图表、绘图、3D、地图、蓝图叠加、图片处理、动效和素材是否符合实施策略
- `RECON-###` 复刻清单中的每个可见元素或效果是否出现在正确区域，并保持同等视觉优先级
- 字体层级、颜色、间距、圆角、边框、阴影和图标
- 响应式变化

验收结果写入 `visual-review.md`。所有 Blocker 和 Major 问题必须由协调 Agent 返修派单给原前端实施 Agent，或新建前端修复 Agent 处理；协调 Agent 不允许自己修改代码。修复完成后重新验收；完成后即停止，不继续开发后端或生产系统。

返修闭环固定为：

```text
视觉验收 Agent -> 协调 Agent -> 前端实施/修复 Agent -> 视觉验收 Agent
```

协调 Agent 可以决定是否接受偏差、是否需要用户确认、把哪些 `VIS-###` 派给哪个 Agent，但不能打开前端文件自己修。
...
| `03-<page-name>-long.png` | 长页面竖向基准图，用于 1:1 前端复现和完整页面截图验收 |
...
6. **按领域确定事实来源**
   `uiux-design.md` 负责文案、字段、状态和交互；批准的图片与 Visual Fidelity Contract 负责构图和视觉样式；前端实施策略负责技术复现方式；`RECON-###` 复刻清单负责逐元素对应；前端预览必须经过独立截图对照验收。

7. **长页面必须连续覆盖**
   先确定页面的完整内容顺序，再生成竖向长页面基准图。滚动页不再生成横向 overview 或独立细节分段图；前端预览必须用设计系统图、竖向长图、比例契约和视觉还原契约复现同一个 UI。

8. **复杂视觉必须先定技术方案**
   前端预览前必须确认 UI 库、图表/绘图/3D/动效方案和素材计划。实施 Agent 不能擅自把已批准 UI 图中的 3D、绘图、蓝图、图表或特殊材质降级为普通静态布局。

9. **前端预览默认按截图复刻**
   已批准页面图不是灵感板，而是复刻目标。实施 Agent 应先匹配结构、比例、层级、元素数量、视觉密度和特殊效果，再补交互；如果实现结果像“参考图启发的新设计”，即使功能可用也不能通过验收。
...
技术复现方式：Frontend Implementation Strategy
逐元素复刻范围：Reference Reconstruction Map / RECON-###
...
| 前端 | 复用或重写路由、组件、设计 tokens、视觉技术方案和 UI 数据模型，并逐步替换 mock adapters |
| 后端 | 根据 API 能力需求设计正式接口，但不把 UI view model 直接当数据库结构 |
| QA | 从交互状态矩阵和 `AC-###` 验收条件生成测试用例 |
| 视觉验收 | 使用参考图、`RECON-###` 复刻清单、完整页面截图、同视口浏览器截图和 `VIS-###` 清单判断预览是否忠实 |
